...
export default function CheckoutPage() {
  ...
  const handleSearchLocation = async () => {
      ...
      setIsSearching(true);
      try {
          const res = await axios.get(`http://127.0.0.1:8000/api/shipping/search?keyword=${searchQuery}`);
  ...
  const handleSaveEditedAddress = async () => {
    ...
    try {
        ...
        if (editingAddressId) {
            await axios.put(`http://127.0.0.1:8000/api/user/addresses/${editingAddressId}`, dataToSend, {
                headers: { Authorization: `Bearer ${token}` },
            });
            ...
            await axios.post(`http://127.0.0.1:8000/api/user/addresses`, dataToSend, {
                headers: { Authorization: `Bearer ${token}` },
            });
  ...
  const fetchRealOngkir = async (destinationId) => {
    ...
    try {
        ...
        for (const c of couriers) {
            try {
                const res = await axios.post("http://127.0.0.1:8000/api/shipping/cost", {
  ...
  const handleCheckout = async () => {
    ...
    try {
        ...
        const response = await axios.post("http://127.0.0.1:8000/api/user/orders/checkout", orderData, {
            headers: { Authorization: `Bearer ${token}` }
        });
        if (response.data) {
            Swal.fire({
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <div className="space-y-4">
                  {checkoutItems.map((item, index) => {
                    ...
                    let itemImg = actualProduct.image_url || actualProduct.image || actualProduct.gambar || null;
                    if (itemImg && !itemImg.startsWith('http')) {
                        itemImg = `http://127.0.0.1:8000${itemImg.startsWith('/') ? '' : '/'}${itemImg}`;
                    }
                    return (
