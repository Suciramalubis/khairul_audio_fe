...
export default function OrderDetailPage() {
  ...
  useEffect(() => {
    const fetchOrderDetail = async () => {
      ...
      try {
        ...
        const response = await axios.get(`http://127.0.0.1:8000/api/user/orders/${id}`, {
  ...
  const updatePaymentMethod = async (method) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(`http://127.0.0.1:8000/api/user/orders/${id}/payment-method`, {
  ...
  const handleCancelOrder = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const token = localStorage.getItem("token");
          await axios.put(`http://127.0.0.1:8000/api/user/orders/${id}/cancel`, {}, {
            headers: { Authorization: `Bearer ${token}` }
          });
                ...
                {isEditingPayment ? (
                    <div className="mt-3 space-y-3">
                        ...
                        <label 
                            ...
                            className={`flex items-center justify-between p-3.5 rounded-lg cursor-pointer border transition-all ${
                                newPaymentMethod === 'QRIS' ? 'bg-blue-50 border-blue-500' : 'bg-white border-gray-200 hover:bg-gray-50'
                            }`}
                        >
                            <div className="flex items-center gap-3">
                            ...
                            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                                newPaymentMethod === 'QRIS' ? 'border-blue-600' : 'border-gray-300'
                            }`}>
                    ...
                    <>
                        {isReviewed ? (
                            ...
                            <button 
                                ...
                                className={`w-full py-3 rounded-lg font-bold transition text-center ${
                                    t.completed 
