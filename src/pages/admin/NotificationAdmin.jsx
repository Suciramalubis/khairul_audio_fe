...
export default function NotificationAdmin() {
  ...
  const handleNotificationClick = async (notif) => {
    if (!notif.isRead) {
        try {
            const token = localStorage.getItem("admin_token") || localStorage.getItem("token");
            await axios.put(`http://127.0.0.1:8000/api/admin/notifications/${notif.id}/read`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
  ...
  const markAllRead = async () => {
    try {
        const token = localStorage.getItem("admin_token") || localStorage.getItem("token");
        await axios.put(`http://127.0.0.1:8000/api/admin/notifications/read-all`, {}, {
            headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleDelete = async (e, id) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                const token = localStorage.getItem("admin_token") || localStorage.getItem("token");
                await axios.delete(`http://127.0.0.1:8000/api/admin/notifications/${id}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
          ...
          <div className="divide-y divide-gray-100">
            {notifications.map((notif) => (
              <div 
                ...
                className={`relative p-5 flex gap-4 transition-colors cursor-pointer group ${
