// Lokasi: src/pages/admin/ProductEditAdmin.jsx
...
export default function ProductEditAdmin() {
  ...
  const removeNewGalleryImage = (indexToRemove) => {
    setNewGalleryImages(prev => prev.filter((_, index) => index !== indexToRemove));
  ...
  if (fetching) {
    return (
      <div className="flex flex-col items-center justify-center py-20 font-sans text-gray-900">
        <div className="w-10 h-10 border-4 border-blue-600 border-t-transparent rounded-full animate-spin mb-4"></div>
        <p className="text-gray-500 font-medium text-sm">Memuat data produk...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto pb-8 font-sans text-gray-900 animate-fadeIn">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Edit Produk</h1>
          <p className="text-sm text-gray-500 mt-0.5">ID: <span className="font-mono text-gray-900">PD{String(id).padStart(3, '0')}</span></p>
        </div>
      </div>
              ...
              <div className="pt-4 border-t border-gray-100">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  {newGalleryPreviews.map((url, index) => (
                    <div key={`new-${index}`} className="relative w-24 h-24 rounded-lg border-2 border-blue-500 overflow-hidden group">
                      <img src={url} alt="New" className="w-full h-full object-cover" />
                      <button type="button" onClick={() => removeNewGalleryImage(index)} className="absolute top-1 right-1 bg-white/90 text-red-600 rounded-md p-1 opacity-0 group-hover:opacity-100">
